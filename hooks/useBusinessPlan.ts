...
const saveBusinessPlanDraft = async (
  targetYear: number,
  name: string,
  data: BusinessPlanDraft,
  locationId: string
): Promise<void> => {
  try {
    const token = localStorage.getItem('auth_token');
...
const deleteBusinessPlanDraft = async (
  draftId: string,
  locationId: string
): Promise<void> => {
  try {
...
export const useBusinessPlan = (
  ...
  const [businessPlanForm, setBusinessPlanForm] =
    useState<BusinessPlanFormState | null>(null);
  const [businessPlanMessage, setBusinessPlanMessage] =
    useState<BusinessPlanMessage | null>(null);
  ...
  const completeYears = useMemo(() => {
    ...
    yearMetrics.forEach((metrics, year) => {
      ...
      if (
        metrics.fatturatoTotale > 0 ||
        metrics.incassato > 0 ||
        metrics.costiFissi > 0 ||
        metrics.costiVariabili > 0
      ) {
        years.push(year);
      }
    });
  ...
  useEffect(() => {
    ...
    const stored = businessPlanDrafts.find(
      draft => draft.targetYear === defaultTarget
    );
    const form = stored
      ...
      setBusinessPlanForm(prev => {
        ...
        switch (field) {
          case 'fatturatoIncrement': {
            ...
            const fatturatoAnnoBase =
              parseNumberInput(next.fatturatoAnnoBase) ?? 0;
            const increment = parseNumberInput(value) ?? 0;
            next.fatturatoPrevisionale = (
              fatturatoAnnoBase *
              (1 + increment / 100)
            ).toFixed(2);
            break;
          ...
          case 'fatturatoValue': {
            ...
            const fatturatoAnnoBaseForIncrement =
              parseNumberInput(next.fatturatoAnnoBase) ?? 0;
            const fatturatoPrevisionaleValue =
              parseNumberInput(cleanCurrencyValue(value)) ?? 0;
            next.fatturatoIncrement =
              fatturatoAnnoBaseForIncrement === 0
                ? '0.00'
                : (
                    (fatturatoPrevisionaleValue /
                      fatturatoAnnoBaseForIncrement -
                      1) *
                    100
                  ).toFixed(2);
            break;
          ...
          case 'incassatoPercent': {
            ...
            const fatturatoPrevisionale =
              parseNumberInput(next.fatturatoPrevisionale) ?? 0;
            const incassatoPercent = parseNumberInput(value) ?? 0;
            next.incassatoPrevisionale = (
              (fatturatoPrevisionale * incassatoPercent) /
              100
            ).toFixed(2);
            break;
          }
          case 'incassatoValue': {
            ...
            const fatturatoPrevisionaleForPercent =
              parseNumberInput(next.fatturatoPrevisionale) ?? 0;
            const incassatoValue =
              parseNumberInput(cleanCurrencyValue(value)) ?? 0;
            next.incassatoPercent =
              fatturatoPrevisionaleForPercent === 0
                ? '0.00'
                : (
                    (incassatoValue / fatturatoPrevisionaleForPercent) *
                    100
                  ).toFixed(2);
            break;
          }
          case 'costiFissiPercent': {
            ...
            const incassatoPrevisionale =
              parseNumberInput(next.incassatoPrevisionale) ?? 0;
            const costiFissiPercent = parseNumberInput(value) ?? 0;
            next.costiFissiPrevisionale = (
              (incassatoPrevisionale * costiFissiPercent) /
              100
            ).toFixed(2);
            break;
          }
          case 'costiFissiValue': {
            ...
            const incassatoPrevisionaleForCostiFissi =
              parseNumberInput(next.incassatoPrevisionale) ?? 0;
            const costiFissiValue =
              parseNumberInput(cleanCurrencyValue(value)) ?? 0;
            next.costiFissiPercent =
              incassatoPrevisionaleForCostiFissi === 0
                ? '0.00'
                : (
                    (costiFissiValue / incassatoPrevisionaleForCostiFissi) *
                    100
                  ).toFixed(2);
            break;
          }
          case 'costiVariabiliPercent': {
            ...
            const incassatoPrevisionaleForVariabili =
              parseNumberInput(next.incassatoPrevisionale) ?? 0;
            const costiVariabiliPercent = parseNumberInput(value) ?? 0;
            next.costiVariabiliPrevisionale = (
              (incassatoPrevisionaleForVariabili * costiVariabiliPercent) /
              100
            ).toFixed(2);
            break;
          }
          case 'costiVariabiliValue': {
            ...
            const incassatoPrevisionaleForCostiVariabili =
              parseNumberInput(next.incassatoPrevisionale) ?? 0;
            const costiVariabiliValue =
              parseNumberInput(cleanCurrencyValue(value)) ?? 0;
            next.costiVariabiliPercent =
              incassatoPrevisionaleForCostiVariabili === 0
                ? '0.00'
                : (
                    (costiVariabiliValue /
                      incassatoPrevisionaleForCostiVariabili) *
                    100
                  ).toFixed(2);
            break;
          }
  ...
  const handleBusinessPlanBaseYearChange = useCallback(
    (year: number) => {
  ...
  const handleBusinessPlanTargetYearChange = useCallback(
    (value: string) => {
  ...
  const handleSaveBusinessPlanDraft = useCallback(() => {
    ...
    const draft: BusinessPlanDraft = {
      ...
      fatturatoPrevisionale:
        parseNumberInput(normalized.fatturatoPrevisionale) ?? 0,
      incassatoPercent: parseNumberInput(normalized.incassatoPercent) ?? 0,
      incassatoPrevisionale:
        parseNumberInput(normalized.incassatoPrevisionale) ?? 0,
      costiFissiPercent: parseNumberInput(normalized.costiFissiPercent) ?? 0,
      costiFissiPrevisionale:
        parseNumberInput(normalized.costiFissiPrevisionale) ?? 0,
      costiVariabiliPercent:
        parseNumberInput(normalized.costiVariabiliPercent) ?? 0,
      costiVariabiliPrevisionale:
        parseNumberInput(normalized.costiVariabiliPrevisionale) ?? 0,
  ...
  const handleDeleteBusinessPlanDraft = useCallback(
    (draftId: string) => {
