...
import {
  fetchFinancialPlanState,
  persistFinancialPlanState,
  fetchFinancialStats,
  saveFinancialStats,
  type FinancialPlanStatePayload,
...
const normalizeCausaliCatalog = (
  catalog: FinancialCausaleGroup[]
): FinancialCausaleGroup[] => {
  const defaultMap = new Map(
    DEFAULT_CAUSALI_CATALOG.map(group => [
      normalizeLabel(group.macroCategory),
      group.macroId,
    ])
  );
  ...
  return catalog.map(group => {
    ...
    let macroId =
      typeof group.macroId === 'number' &&
      !Number.isNaN(group.macroId) &&
      group.macroId > 0
        ? group.macroId
...
export const useFinancialPlanData = (locationId?: string) => {
  ...
  const [dbConsuntivoOverrides, setDbConsuntivoOverrides] =
    useState<PlanOverrides>({});
  ...
  const basePlanByYear = useMemo(() => {
    const source =
      causaliCatalog.length > 0 ? causaliCatalog : DEFAULT_CAUSALI_CATALOG;
  ...
  const getPlanConsuntivoValue = useCallback(
    (
      ...
      const override =
        dbConsuntivoOverrides[macro]?.[category]?.[detail]?.[monthKey];
      if (override !== undefined) {
        return override;
      }

      // Get sum from data entries (InserisciDati)
      const dataEntriesSum = getSumForCausale(
        macro,
        category,
        detail,
        year,
  ...
  const setOverride = useCallback(
    async (
      ...
      const setter =
        target === 'preventivo' ? setDbPlanOverrides : setDbConsuntivoOverrides;
      setter(prev => {
        const next = { ...prev } as PlanOverrides;
        if (!next[macro]) next[macro] = {} as any;
        if (!next[macro][category]) next[macro][category] = {} as any;
        if (!next[macro][category][detail])
          next[macro][category][detail] = {} as any;
        if (value === null) {
      ...
      try {
        const currentPlanOverrides =
          target === 'preventivo'
            ? {
                ...dbPlanOverrides,
                [macro]: {
                  ...dbPlanOverrides[macro],
                  [category]: {
                    ...dbPlanOverrides[macro]?.[category],
                    [detail]: {
                      ...dbPlanOverrides[macro]?.[category]?.[detail],
                      [monthKey]: value,
                    },
                  },
                },
            ...
            : dbPlanOverrides;
        const currentConsuntivoOverrides =
          target === 'consuntivo'
            ? {
                ...dbConsuntivoOverrides,
                [macro]: {
                  ...dbConsuntivoOverrides[macro],
                  [category]: {
                    ...dbConsuntivoOverrides[macro]?.[category],
                    [detail]: {
                      ...dbConsuntivoOverrides[macro]?.[category]?.[detail],
                      [monthKey]: value,
                    },
                  },
                },
  ...
  const handleSavePlan = useCallback(
    async (selectedYear: number, _dirtyKeys: Set<string>) => {
      ...
      try {
        ...
        const buildAudit = (
          ...
          const out: any[] = [];
          const source =
            target === 'preventivo' ? dbPlanOverrides : dbConsuntivoOverrides;
        ...
        const statsToSave = convertStatsOverridesToRows(
          dbStatsOverrides,
          selectedYear
        );
  ...
  const handleCancelPlan = useCallback(async () => {
    ...
    try {
      ...
      if (payload) {
        setDbPlanOverrides(payload.preventivoOverrides ?? {});
        setDbConsuntivoOverrides(
          (payload as FinancialPlanStatePayload | null)?.consuntivoOverrides ??
            {}
        );
        setDbStatsOverrides(payload.statsOverrides ?? {});
        setMonthlyMetrics(payload.monthlyMetrics ?? []);
        setCausaliCatalog(() =>
          normalizeCausaliCatalog(
            payload &&
              payload.causaliCatalog &&
              payload.causaliCatalog.length > 0
              ? (payload.causaliCatalog as FinancialCausaleGroup[])
              : DEFAULT_CAUSALI_CATALOG
          )
        );
  ...
  const handleCausaliPersist = useCallback(
    async (next: FinancialCausaleGroup[]) => {
      setSavingState(true);
      try {
  ...
  const setStatsOverrides = useCallback(
    async (newStatsOverrides: StatsOverrides) => {
      if (!locationId) return;
      ...
      try {
        const payload: FinancialPlanStatePayload = {
          ...
          manualLog: [],
          monthlyMetrics: monthlyMetrics,
          statsOverrides: newStatsOverrides as unknown as Record<
            string,
