...
export const StatsTable: React.FC<StatsTableProps> = ({
  ...
  const { rows: statsTableData, subtotalsByYear, periodTotal } = useMemo(() => {
    ...
    availableYears.forEach((year) => {
      for (let monthIndex = 0; monthIndex < 12; monthIndex += 1) {
        ...
        const fatturatoPrevisionale = getFieldValue(dataWithKey, 'fatturatoPrevisionale') ?? null;
  ...
  const renderSubtotalRow = (subtotal: any, isPeriodTotal: boolean = false) => (
    <tr key={subtotal.monthKey} className="bg-gradient-to-r from-slate-100 to-slate-200 border-t-2 border-gray-400">
      ...
      <td className="px-4 py-3 text-right text-sm font-bold text-gray-900">
        <span className={getValueStyle(subtotal.fatturatoImponibile, 'font-bold')}>
      ...
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-blue-100 to-blue-200">
        <span className={getValueStyle(subtotal.fatturatoTotale, 'font-bold text-blue-900')}>
          {formatCurrencyValue(subtotal.fatturatoTotale)}
        </span>
      </td>
      <td className="px-4 py-3 text-right text-sm font-bold text-gray-900">
        <span className={getValueStyle(subtotal.fatturatoPrevisionale, 'font-bold')}>
          {formatCurrencyValue(subtotal.fatturatoPrevisionale)}
        </span>
      </td>
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-blue-100 to-blue-200">
        <span className={getValueStyle(subtotal.incassato, 'font-bold text-blue-900')}>
      ...
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-emerald-100 to-emerald-200">
        <span className={getValueStyle(subtotal.incassatoPrevisionale, 'font-bold text-emerald-900')}>
      ...
      <td className="px-4 py-3 text-right text-sm font-bold text-gray-900">
        <span className={getValueStyle(subtotal.saldoSecondoConto, 'font-bold')}>
          {formatCurrencyValue(subtotal.saldoSecondoConto)}
        </span>
      </td>
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-blue-100 to-blue-200">
        <span className={getValueStyle(subtotal.saldoTotale, 'font-bold text-blue-900')}>
      ...
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-emerald-100 to-emerald-200">
        <span className={getValueStyle(subtotal.utile, 'font-bold text-emerald-900')}>
      ...
      <td className="px-4 py-3 text-right text-sm font-bold bg-gradient-to-r from-emerald-200 to-emerald-300">
        <span className={getValueStyle(subtotal.utilePrevisionale, 'font-bold text-emerald-900')}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-blue-700">
              ...
              {availableYears.length > 1 && ` + ${availableYears.length} subtotali + 1 totale`}
            </span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm min-w-max">
            ...
            <tbody className="divide-y divide-gray-200">
              {statsTableData.map((row, index) => {
                const isLastMonthOfYear = index === statsTableData.length - 1 || 
                  statsTableData[index + 1]?.year !== row.year;
                const subtotal = subtotalsByYear.get(row.year);
                
                return (
                  <React.Fragment key={row.monthKey}>
                    <tr className={`hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'corrispettivi'), 'font-semibold')}>
                  ...
                  {editMode ? (
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'fatturatoImponibile'), 'font-semibold')}>
                  ...
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-blue-25 to-blue-50">
                    <span className={getValueStyle(row.fatturatoTotale, 'font-bold text-blue-800')}>
                  ...
                  <td className="px-4 py-3 text-right text-sm font-medium text-emerald-800">
                    {editMode ? (
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'fatturatoPrevisionale'), 'font-semibold text-emerald-700')}>
                    {formatCurrencyValue(getRowFieldValue(row, 'fatturatoPrevisionale'))}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-blue-25 to-blue-50">
                    <span className={getValueStyle(row.incassato, 'font-bold text-blue-800')}>
                  ...
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-emerald-25 to-emerald-50">
                    <span className={getValueStyle(row.incassatoPrevisionale, 'font-bold text-emerald-800')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'saldoConto'), 'font-semibold')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'saldoSecondoConto'), 'font-semibold')}>
                        {formatCurrencyValue(getRowFieldValue(row, 'saldoSecondoConto'))}
                      </span>
                  )}
                </td>
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-blue-25 to-blue-50">
                    <span className={getValueStyle(row.saldoTotale, 'font-bold text-blue-800')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'creditiPendenti'), 'font-semibold text-orange-700')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'creditiScaduti'), 'font-semibold text-red-700')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'debitiFornitore'), 'font-semibold text-purple-700')}>
                  ...
                  {editMode ? (
                    ...
                    />
                      ...
                      <span className={getValueStyle(getRowFieldValue(row, 'debitiBancari'), 'font-semibold text-purple-700')}>
                        {formatCurrencyValue(getRowFieldValue(row, 'debitiBancari'))}
                      </span>
                  )}
                </td>
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-emerald-25 to-emerald-50">
                    <span className={getValueStyle(row.utile, 'font-bold text-emerald-800')}>
                    {formatCurrencyValue(row.utile)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right text-sm bg-gradient-to-r from-emerald-50 to-emerald-100">
                    <span className={getValueStyle(row.utilePrevisionale, 'font-bold text-emerald-900')}>
                    ...
                    {isLastMonthOfYear && subtotal && renderSubtotalRow(subtotal, false)}
                  </React.Fragment>
                );
              })}
