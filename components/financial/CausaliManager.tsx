...
export const CausaliManager: React.FC<CausaliManagerProps> = ({
  ...
  const handleAddMacro = () => {
    ...
    if (!macro) return;
    onCausaliPersist([...causaliCatalog, { macroCategory: macro, categories: [] }]);
  };
  ...
  const handleRenameCategory = (macroIndex: number, categoryIndex: number, currentName: string) => {
    const name = window.prompt('Rinomina categoria', currentName) ?? currentName;
    const next = [...causaliCatalog];
    next[macroIndex].categories[categoryIndex] = { ...next[macroIndex].categories[categoryIndex], name };
    onCausaliPersist(next);
  };
  ...
  const handleRenameCausale = (macroIndex: number, categoryIndex: number, causaleIndex: number, currentName: string) => {
    ...
    next[macroIndex].categories[categoryIndex] = { ...next[macroIndex].categories[categoryIndex], items };
    onCausaliPersist(next);
  };

  const handleDeleteCausale = (macroIndex: number, categoryIndex: number, causaleIndex: number) => {
