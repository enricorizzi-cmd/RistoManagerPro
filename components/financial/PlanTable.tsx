...
export const PlanTable: React.FC<PlanTableProps> = ({
  ...
  const getMonthsToShow = () => {
    if (!periodoMode) {
      ...
      while (
        currentYear < selectedToYear ||
        (currentYear === selectedToYear && currentMonth <= selectedToMonth)
      ) {
        months.push({
    ...
    (
      ...
      if (!periodoMode) {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex += 1) {
          const p = getPlanPreventivoValue(
            macro,
            category,
            detail,
            year,
          ...
          const c = getPlanConsuntivoValue(
            macro,
            category,
            detail,
            year,
    ...
    <div className="overflow-y-auto rounded-2xl bg-white p-5 shadow-sm max-h-[80vh]">
      <table className="w-full text-sm">
        <thead className="bg-slate-50 text-xs uppercase tracking-wide text-gray-600">
          <tr>
            <th className="px-3 py-3 text-left bg-slate-50 sticky top-0 left-0 z-30 w-48">
              CATEGORIA
            </th>
            <th className="px-3 py-3 text-center border-l-2 border-gray-300 bg-slate-50 sticky top-0 z-20">
              SOMMA PROGRESSIVA
            </th>
            <th className="px-3 py-3 text-center bg-slate-50 sticky top-0 z-20">
            ...
            {monthsToShow.map(month => (
              <th
                ...
                className="px-3 py-3 text-center border-l-2 border-gray-300 bg-slate-50 sticky top-0 z-20"
                colSpan={onlyConsuntivo ? 1 : 2}
              >
          ...
          <tr>
            ...
            <th className="px-3 py-2 text-center text-xs font-normal border-l-2 border-gray-300 bg-slate-50 sticky top-[3rem] z-20">
              TOTALE
            </th>
            <th className="px-3 py-2 text-center text-xs font-normal bg-slate-50 sticky top-[3rem] z-20">
              %
            </th>
            {showPrevisionaleTotals && (
              <>
        ...
        <tbody className="divide-y divide-gray-100">
          {causaliCatalog.map(group => (
            <React.Fragment key={group.macroCategory}>
              <tr
                className={`${getMacroColor(group.macroCategory)} text-sm font-bold uppercase`}
              >
                <td className="px-3 py-3 sticky left-0 bg-inherit z-10 w-48">
                  {group.macroCategory}
                </td>
                {(() => {
                  const macroSum = group.categories.reduce((acc, cat) => {
                    const macro = planYear?.macros.find(
                      m => m.macro === group.macroCategory
                    );
                    const categoryDetails =
                      macro?.details?.filter(d => d.category === cat.name) ??
                      [];
                    return (
                      acc +
                  ...
                  if (group.macroId === 1) {
                    ...
                    totalIncassato = monthsToShow.reduce(
                      (acc, month) =>
                        acc +
                        getIncassatoTotal(
                          causaliCatalog,
                ...
                {showPrevisionaleTotals &&
                  (() => {
                    const macroPrevisionaleSum = group.categories.reduce(
                      (acc, cat) => {
                        const macro = planYear?.macros.find(
                          m => m.macro === group.macroCategory
                        );
                        const categoryDetails =
                          macro?.details?.filter(
                    ...
                    if (group.macroId === 1) {
                      ...
                      totalIncassatoPrevisionale = monthsToShow.reduce(
                        (acc, month) =>
                          acc +
                          getIncassatoTotal(
                            causaliCatalog,
                    ...
                    const percentagePrevisionale =
                      totalIncassatoPrevisionale === 0
                        ? 0
                        : (macroPrevisionaleSum / totalIncassatoPrevisionale) *
                          100;
                ...
                {monthsToShow.map(month => {
                  ...
                  return (
                    <React.Fragment
                      key={`macro-header-${month.year}-${month.monthIndex}`}
                    >
                      {!onlyConsuntivo && (
                        <td className="px-3 py-3 text-right border-l-2 border-gray-300">
              ...
              {group.categories.map(category => {
                ...
                const categoryDetails =
                  macro?.details?.filter(d => d.category === category.name) ??
                  [];
                ...
                return (
                  <React.Fragment
                    key={`${group.macroCategory}-${category.name}`}
                  >
                    {/* Subtotale categoria */}
                    <tr className="bg-slate-50 font-semibold">
                      <td className="px-3 py-2 text-sm text-gray-700 sticky left-0 bg-slate-50 z-10 w-48">
                        {category.name}
                      </td>
                      {(() => {
                        const categorySum = categoryDetails.reduce(
                          (acc, d) =>
                            acc +
                            monthsToShow.reduce(
                              (monthAcc, month) =>
                        ...
                        const totalIncassato = monthsToShow.reduce(
                          (acc, month) =>
                            acc +
                            getIncassatoTotal(
                              causaliCatalog,
                      ...
                      {showPrevisionaleTotals &&
                        (() => {
                          ...
                          const totalIncassatoPrevisionale =
                            monthsToShow.reduce(
                              (acc, month) =>
                                acc +
                                getIncassatoTotal(
                          ...
                          const totalPercentagePrevisionale =
                            totalIncassatoPrevisionale === 0
                              ? 0
                              : (categoryPrevisionaleSum /
                                  totalIncassatoPrevisionale) *
                                100;
                            ...
                            <>
                              <td className="px-3 py-2 text-right text-sm border-l-2 border-gray-200">
                                <div className="font-semibold text-purple-700">
                                  {formatCurrencyValue(categoryPrevisionaleSum)}
                                </div>
                              </td>
                              <td className="px-3 py-2 text-right text-sm">
                                <div className="font-semibold text-purple-700">
                                  {totalPercentagePrevisionale.toFixed(1)}%
                                </div>
                              </td>
                            </>
                          );
                      ...
                      {monthsToShow.map(month => {
                        const p = categoryDetails.reduce(
                          (acc, d) =>
                            acc +
                            getPlanPreventivoValue(
                              group.macroCategory,
                        ...
                        const c = categoryDetails.reduce(
                          (acc, d) =>
                            acc +
                            getPlanConsuntivoValue(
                              group.macroCategory,
                        ...
                        return (
                          <React.Fragment
                            key={`subtotal-${month.year}-${month.monthIndex}`}
                          >
                            {!onlyConsuntivo && (
                              <td className="px-3 py-2 text-right text-sm border-l-2 border-gray-200">
                    ...
                    {category.items.map(causale => {
                      ...
                      const hasCausaleValue = rowHasAnyValue(
                        group.macroCategory,
                        category.name,
                        causale,
                        selectedYear
                      ...
                      return (
                        <tr
                          key={`${category.name}-${causale}`}
                          className="hover:bg-slate-50"
                        >
                          <td className="px-3 py-2 text-sm text-gray-700 pl-6 sticky left-0 bg-white z-10 w-48 hover:bg-slate-50">
                            {causale}
                          </td>
                          {(() => {
                            const causaleSum = monthsToShow.reduce(
                              (acc, month) =>
                                acc +
                                getPlanConsuntivoValue(
                                  group.macroCategory,
          ...
          <tr className="bg-emerald-50 text-sm font-bold uppercase text-emerald-800">
            <td
              className="px-3 py-3 sticky left-0 bg-emerald-50 z-10 w-48"
              colSpan={
                3 +
                (showPrevisionaleTotals ? 2 : 0) +
                monthsToShow.length * (onlyConsuntivo ? 1 : 2)
              }
            >
              UTILE DI CASSA
            </td>
          ...
          <tr className="bg-emerald-100 font-semibold">
            <td className="px-3 py-2 text-sm text-gray-700 sticky left-0 bg-emerald-100 z-10 w-48">
              Utile di cassa
            </td>
            {(() => {
              ...
              const totalPercentage =
                totalIncassato === 0
                  ? 0
                  : (utileCassaSum / totalIncassato) * 100;

              return (
                <>
                  <td className="px-3 py-2 text-right text-sm border-l-2 border-gray-200">
                    <div className="font-semibold text-emerald-700">
                      {formatCurrencyValue(utileCassaSum)}
                    </div>
                  </td>
                  <td className="px-3 py-2 text-right text-sm">
                    <div className="font-semibold text-emerald-700">
            ...
            {showPrevisionaleTotals &&
              (() => {
                const utileCassaPrevisionaleSum = monthsToShow.reduce(
                  (acc, month) => {
                    return (
                      acc +
                      calculateUtileFromMacroTotals(
                ...
                const totalIncassatoPrevisionale = monthsToShow.reduce(
                  (acc, month) =>
                    acc +
                    getIncassatoTotal(
                      causaliCatalog,
                ...
                const totalPercentagePrevisionale =
                  totalIncassatoPrevisionale === 0
                    ? 0
                    : (utileCassaPrevisionaleSum / totalIncassatoPrevisionale) *
                      100;
                ...
                return (
                  <>
                    <td className="px-3 py-2 text-right text-sm border-l-2 border-gray-200">
                      <div className="font-semibold text-purple-700">
                        {formatCurrencyValue(utileCassaPrevisionaleSum)}
                      </div>
                    </td>
                    <td className="px-3 py-2 text-right text-sm">
                      <div className="font-semibold text-purple-700">
            ...
            {monthsToShow.map(month => {
              ...
              const utileCassaPreventivo = calculateUtileFromMacroTotals(
                causaliCatalog,
                planYear,
                getPlanPreventivoValue,
                month.year,
              ...
              const utileCassaConsuntivo = calculateUtileFromMacroTotals(
                causaliCatalog,
                planYear,
                getPlanConsuntivoValue,
                month.year,
              ...
              return (
                <React.Fragment
                  key={`utile-cassa-${month.year}-${month.monthIndex}`}
                >
                  {!onlyConsuntivo && (
                    <td className="px-3 py-2 text-right text-sm border-l-2 border-gray-200">
                      <div className="font-semibold text-sky-700">
                        {formatCurrencyValue(utileCassaPreventivo)}
                      </div>
                    </td>
                  )}
                  <td className="px-3 py-2 text-right text-sm">
                    <div className="font-semibold text-emerald-700">
                      {formatCurrencyValue(utileCassaConsuntivo)}
                    </div>
                  </td>
                </React.Fragment>
              );
