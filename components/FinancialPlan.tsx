...
const FinancialPlan: React.FC = () => {
  ...
  const [isBusinessPlanLoading, setIsBusinessPlanLoading] =
  ...
  const availableYears = useMemo(() => {
    return hookAvailableYears.length > 0
      ? hookAvailableYears
      : [new Date().getFullYear()];
  }, [hookAvailableYears]);
  ...
  useEffect(() => {
    if (!basePlanByYear.has(selectedYear) && availableYears.length > 0) {
      ...
      const closestYear =
        availableYears.find(year => basePlanByYear.has(year)) ||
        availableYears[0];
  ...
  const applyBusinessPlanToOverrides = async () => {
    if (!businessPlanForm || businessPlanForm.baseYear === null) {
      showNotification(
        'Compila il Business Plan prima di applicare il previsionale.',
        'error'
      );
      return;
    }
    ...
    if (!planBase || !metrics) {
      showNotification(
        "Dati storici insufficienti per l'anno base selezionato.",
        'error'
      );
      return;
    }
    ...
    const fatturatoPrevisionale =
      parseNumberInput(businessPlanForm.fatturatoPrevisionale) ??
      metrics.fatturatoTotale;
    const incassatoPrevisionale =
      parseNumberInput(businessPlanForm.incassatoPrevisionale) ??
      metrics.incassato;
    const costiFissiPrevisionale =
      parseNumberInput(businessPlanForm.costiFissiPrevisionale) ??
      metrics.costiFissi;
    const costiVariabiliPrevisionale =
      parseNumberInput(businessPlanForm.costiVariabiliPrevisionale) ??
      metrics.costiVariabili;

    // Controlla se ci sono override esistenti per l'anno target
    const hasExistingOverrides =
      Object.keys(planOverrides).some(key => key.includes(`|${targetYear}|`)) ||
      Object.keys(statsOverrides).some(key => key.includes(`|${targetYear}|`));
    ...
    const fatturatoRatios =
      baseMetrics.monthlyFatturato && baseMetrics.fatturatoTotale > 0
        ? baseMetrics.monthlyFatturato.map(
            value => value / baseMetrics.fatturatoTotale
          )
        : new Array(12).fill(1 / 12);

    const incassatoRatios =
      baseMetrics.monthlyIncassato && baseMetrics.incassato > 0
        ? baseMetrics.monthlyIncassato.map(
            value => value / baseMetrics.incassato
          )
        : new Array(12).fill(1 / 12);

    const costiFissiRatios =
      baseMetrics.monthlyCostiFissi && baseMetrics.costiFissi > 0
        ? baseMetrics.monthlyCostiFissi.map(
            value => value / baseMetrics.costiFissi
          )
        : new Array(12).fill(1 / 12);

    const costiVariabiliRatios =
      baseMetrics.monthlyCostiVariabili && baseMetrics.costiVariabili > 0
        ? baseMetrics.monthlyCostiVariabili.map(
            value => value / baseMetrics.costiVariabili
          )
        : new Array(12).fill(1 / 12);

    // Distribuisci i valori annuali sui mesi
    const incassatoMensile = distributeAnnualValueToMonths(
      incassatoPrevisionale,
      incassatoRatios
    );
    const costiFissiMensile = distributeAnnualValueToMonths(
      costiFissiPrevisionale,
      costiFissiRatios
    );
    const costiVariabiliMensile = distributeAnnualValueToMonths(
      costiVariabiliPrevisionale,
      costiVariabiliRatios
    );
      ...
      for (const causale of causaliValues) {
        if (causale.totalValue === 0) {
          for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
            await setOverride(
              'preventivo',
              causale.detail.macro,
              causale.detail.category,
              causale.detail.detail,
              targetYear,
              monthIndex,
              0
            );
        ...
        const causaleShare =
          totalCausaliValue === 0 ? 0 : causale.totalValue / totalCausaliValue;
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          const monthValue = round2(
            causaleAnnualTarget * causaleMonthlyRatios[monthIndex]
          );
          await setOverride(
            'preventivo',
            causale.detail.macro,
    ...
    for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
      ...
      filteredStatsOverrides[`${monthKey}|fatturatoPrevisionale`] = round2(
        fatturatoPrevisionale * fatturatoRatios[monthIndex]
      );
      filteredStatsOverrides[`${monthKey}|incassatoPrevisionale`] =
        incassatoMensile[monthIndex];
      filteredStatsOverrides[`${monthKey}|utilePrevisionale`] = round2(
        incassatoMensile[monthIndex] -
          costiFissiMensile[monthIndex] -
          costiVariabiliMensile[monthIndex]
      );
    }
    ...
    const updatedStatsOverrides = {
      ...statsOverrides,
      ...filteredStatsOverrides,
    };
    ...
    if (currentLocation?.id) {
      try {
        ...
        const statsToSave = statsRows.sort(
          (a, b) => a.monthIndex - b.monthIndex
        );
        await saveFinancialStats(currentLocation.id, statsToSave);

        showNotification(
          `Previsionale ${targetYear} applicato e salvato nel database.`,
          'success'
        );
      } catch (error) {
        console.error('❌ Errore nel salvataggio:', error);
        showNotification(
          `Previsionale ${targetYear} applicato ma errore nel salvataggio.`,
          'error'
        );
      }
    }
  ...
  const handleResetBusinessPlan = async () => {
    ...
    if (currentLocation?.id) {
      try {
        ...
        showNotification(
          `Previsionale ${targetYear} ripristinato e salvato nel database.`,
          'success'
        );
      } catch (error) {
        console.error('❌ Errore nel reset:', error);
        showNotification(
          `Errore nel ripristino del previsionale ${targetYear}.`,
          'error'
        );
      }
    }
            ...
            {!periodoMode ? (
              ...
              <>
                ...
                <select
                  ...
                  onChange={event =>
                    setSelectedFromYear(Number(event.target.value))
                  }
                  className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  {availableYears.map(year => (
                ...
                <select
                  ...
                  onChange={event =>
                    setSelectedToMonth(Number(event.target.value))
                  }
                  className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  {[
                ...
                <select
                  ...
                  onChange={event =>
                    setSelectedToYear(Number(event.target.value))
                  }
                  className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  {availableYears.map(year => (
        ...
        <div className="space-y-4">
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (success) {
                        ...
                        showNotification(
                          'Statistiche salvate con successo.',
                          'success'
                        );
                      }
                    } catch (error) {
          ...
          <StatsTable
            availableYears={Array.from(
              { length: statsToYear - statsFromYear + 1 },
              (_, i) => statsFromYear + i
            )}
            statsOverrides={statsOverrides}
            financialStatsRows={financialStatsRows}
