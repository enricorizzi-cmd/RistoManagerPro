...
import { RestaurantLocation, AppContextType, AppNotification, NotificationType } from '../types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateLocationSettings = async (locationId: string, newSettings: RestaurantLocation) => {
    try {
      const response = await fetch(`http://localhost:4000/api/locations/${locationId}`, {
        method: 'PUT',
        headers: {
