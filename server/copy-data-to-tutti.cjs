...
function getActiveLocations() {
  return new Promise((resolve, reject) => {
    masterDb.all(
      'SELECT id, name FROM locations WHERE status = "active" AND id != "all"',
      (err, rows) => {
        if (err) {
          reject(err);
        } else {
...
async function aggregateData() {
  try {
        ...
        if (financialState) {
          ...
          if (financialState.preventivoOverrides) {
            Object.keys(financialState.preventivoOverrides).forEach(key => {
              ...
              Object.keys(financialState.preventivoOverrides[key]).forEach(
                subKey => {
                  const currentValue =
                    parseFloat(
                      aggregatedData.preventivoOverrides[key][subKey]
                    ) || 0;
                  const newValue =
                    parseFloat(
                      financialState.preventivoOverrides[key][subKey]
                    ) || 0;
                  aggregatedData.preventivoOverrides[key][subKey] =
          ...
          if (financialState.consuntivoOverrides) {
            Object.keys(financialState.consuntivoOverrides).forEach(key => {
              ...
              Object.keys(financialState.consuntivoOverrides[key]).forEach(
                subKey => {
                  const currentValue =
                    parseFloat(
                      aggregatedData.consuntivoOverrides[key][subKey]
                    ) || 0;
                  const newValue =
                    parseFloat(
                      financialState.consuntivoOverrides[key][subKey]
                    ) || 0;
                  aggregatedData.consuntivoOverrides[key][subKey] =
                    currentValue + newValue;
                }
          ...
          if (financialState.statsOverrides) {
            Object.keys(financialState.statsOverrides).forEach(key => {
              const currentValue =
                parseFloat(aggregatedData.statsOverrides[key]) || 0;
              const newValue =
                parseFloat(financialState.statsOverrides[key]) || 0;
          ...
          if (financialState.monthlyMetrics) {
            financialState.monthlyMetrics.forEach(metric => {
              const existingMetric = aggregatedData.monthlyMetrics.find(
                m =>
          ...
          if (
            financialState.causaliCatalog &&
            financialState.causaliCatalog.length > 0
          ) {
          ...
          console.log(
            `   ⚠️  ${location.name}: nessun dato finanziario trovato`
          );
        }
      } catch (error) {
    ...
    console.log(
      `   📊 Locations processate: ${processedLocations}/${locations.length}`
    );
    ...
    if (changes > 0) {
      ...
      console.log(
        `   - Preventivo Overrides: ${Object.keys(aggregatedData.preventivoOverrides).length} chiavi`
      );
      console.log(
        `   - Consuntivo Overrides: ${Object.keys(aggregatedData.consuntivoOverrides).length} chiavi`
      );
      console.log(
        `   - Stats Overrides: ${Object.keys(aggregatedData.statsOverrides).length} chiavi`
      );
      console.log(
        `   - Monthly Metrics: ${aggregatedData.monthlyMetrics.length} record`
      );
      console.log(
        `   - Causali Catalog: ${aggregatedData.causaliCatalog.length} voci`
      );
    } else {
