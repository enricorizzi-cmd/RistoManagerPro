...
const masterDb = new sqlite3.Database(
  path.join(__dirname, 'data', 'master.db')
);

console.log('1. Locations nel database master:');
masterDb.all(
  'SELECT id, name, status FROM locations ORDER BY name',
  (err, rows) => {
    if (err) {
    ...
    rows
      ...
      .forEach(location => {
        const dbPath = path.join(
          __dirname,
          'data',
          `ristomanager_${location.id}.db`
        );
        const db = new sqlite3.Database(dbPath);

        db.get(
          'SELECT COUNT(*) as count FROM financial_plan_state',
          (err, result) => {
            if (err) {
              console.log(`   ❌ ${location.name}: Database non accessibile`);
            } else {
              console.log(
                `   ✅ ${location.name}: ${result.count} stati finanziari`
              );

              // Se ci sono dati, mostra i dettagli
              if (result.count > 0) {
                db.get(
                  'SELECT id, updated_at FROM financial_plan_state LIMIT 1',
                  (err, row) => {
                    if (!err && row) {
    ...
    allDb.get(
      ...
      (err, result) => {
        if (err) {
          ...
          if (result.count > 0) {
            allDb.get(
              'SELECT id, data, updated_at FROM financial_plan_state LIMIT 1',
              (err, row) => {
                if (!err && row) {
                  console.log(`      - ID: ${row.id}`);
                  console.log(`      - Aggiornato: ${row.updated_at}`);
                  try {
                    ...
                    console.log(
                      `      - Preventivo Overrides: ${Object.keys(data.preventivoOverrides || {}).length} chiavi`
                    );
                    console.log(
                      `      - Consuntivo Overrides: ${Object.keys(data.consuntivoOverrides || {}).length} chiavi`
                    );
                    console.log(
                      `      - Stats Overrides: ${Object.keys(data.statsOverrides || {}).length} chiavi`
                    );
                    console.log(
                      `      - Monthly Metrics: ${(data.monthlyMetrics || []).length} record`
                    );
                  } catch (e) {
