...
const masterDb = {
  async query(sql, params = []) {
    ...
    throw new Error(
      'Complex SQL queries not supported. Use specific Supabase methods.'
    );
  },
  ...
  async run(table, data, options = {}) {
    if (options.operation === 'insert') {
      return await supabaseRequest('POST', table, {
        data,
        upsert: options.upsert,
      });
    } else if (options.operation === 'update') {
      return await supabaseRequest('PATCH', table, {
        data,
        filters: options.filters,
      });
    } else if (options.operation === 'delete') {
      return await supabaseRequest('DELETE', table, {
        filters: options.filters,
      });
    }
