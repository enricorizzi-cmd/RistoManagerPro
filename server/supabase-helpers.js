...
async function supabaseFetch(method, table, options = {}) {
  ...
  try {
    ...
    console.error(
      `Supabase request error (${method} ${table}):`,
      error.message
    );
    throw error;
  }
...
const masterDbHelpers = {
  ...
  async query(sql, params = []) {
    ...
    if (countMatch) {
      ...
      const result = await supabaseFetch('GET', table, {
        select: 'id',
        limit: 1000,
      });
      return [
    ...
    throw new Error(
      `Complex SQL query not supported: ${sql}. Use specific Supabase methods.`
    );
  },
  ...
  async get(sql, params = []) {
    ...
    if (selectMatch) {
      ...
      return await supabaseFetch('GET', table, {
        select,
        filters,
        single: true,
      });
